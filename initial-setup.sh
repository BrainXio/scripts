#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# initial-setup.sh — Create non-root sudo user, harden SSH, and optimize for CI/CD.
...
[[ -n "$ZSH_BIN" && ! $(grep -Fx "$ZSH_BIN" /etc/shells) ]] && echo "$ZSH_BIN" >> /etc/shells

# Get valid username
while true; do
    read -rp "Enter username (lowercase, start with letter): " username
...
if [[ -z "${CI_PASSWORD:-}" ]]; then
    while true; do
        read -rsp "Enter password: " password1; echo
...
default_shell=${ZSH_BIN:-/bin/bash}
useradd -m -s "$default_shell" -G sudo,docker "$username"
